...
#define FIXED_KEY \
    size_t i;\
    uint8_t fixed_key[20];\
    memcpy(fixed_key, key.bytes, key.length);\
    for (i = key.length; i < 20; ++i) fixed_key[i] = 0;\
...
//  无符号位 0 有符号位1
    ...
    key_array  = xxtea_to_uint_array(key, 20, 0, &key_len);
    if (!key_array) {
        free(data_array);
        return NULL;
    }
...
//    return [[self encrypt:data stringKey:key sign:sign] base64EncodedStringWithOptions:0];
//    return [[NSString alloc] initWithData:[self encrypt:data stringKey:key sign:sign] encoding:NSUTF8StringEncoding];
}
